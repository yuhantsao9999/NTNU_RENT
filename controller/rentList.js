...
const rentList = async (files, data) => {
    ...
    const set = Array.from(
        data
            ...
            .replace(re, '')
            .split(',')
    ).concat([paths.toString()]);
    console.log(set);
    const user_id = await mysql.query('SELECT user_id FROM Users WHERE email = ?', set[0]).catch((err) => {
        console.log(err);
        return false;
    });
    console.log('rentList user_id', user_id);

    set.splice(0, 1);
    set.unshift(user_id[0].user_id);

    const sql =
        'INSERT INTO Product(user_id, p_name, category, brand, price, days, intro, place, photo) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?)';
    const result = await mysql.query(sql, set).catch((err) => {
        console.log('rentList err', err);
        return false;
    });
    if (result) {
