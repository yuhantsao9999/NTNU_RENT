...
const shop = async (min, max, brand, order) => {
    ...
    var sql =
        ...
        "WHERE price >= ? AND price <= ? AND Product.p_status = 'waiting' AND Product.product_id NOT IN (SELECT DISTINCT C.product_id FROM Contract as C WHERE C.c_status = 'continue') ";
    ...
    const results = await mysql.query(sql, query).catch((err) => {
        console.log(err);
    });
    const data = {
        product_id: [],
